...
function convertToRoman(num) {
    ...
    var str = '';
// i will be a key in roman
    for (var i of Object.keys(roman)) {
    // check how many times current roman fits into num
    // if none, q=0
        var q = Math.floor(num / roman[i]);
    // subtract our multiplier (q) times our current val
        num -= q * roman[i];
    // append roman num to str q times
        str += i.repeat(q);
    }
...
The user may fill out the form field any way they choose as long as it has the format of a valid US 
number. The following are examples of valid formats for US numbers (refer to the tests below for 
other variants): 
555-555-5555
(555)555-5555
(555) 555-5555
...
function telephoneCheck(str) {
  let r = /^(1\s?)?(\d{3}\s?|\(\d{3}\)\s?|\d{3}-)(\d{3})(\s|-)?(\d{4})$/g
  return r.test(str);
}
//
